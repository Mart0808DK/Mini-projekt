...
function CreateNewUser({create}) {
    async function handleSubmit(e) {
        ...
        const user = {id: e.target.id.value, name: e.target.name.value, age: e.target.age.value}
        await create(user)
        e.target.reset();
    }
    return (
        <div className="flex justify-center items-center">
            <form onSubmit={handleSubmit} className="flex flex-col gap-2 w-80">
                ...
                <input className="text-slate-950" id="id-input" type="number" placeholder="id" name="id"/>
